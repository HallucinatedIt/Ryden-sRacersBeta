...
function buildScenery(W,def,P,Q,H){
  ...
  const scatter=(n,minD,maxD,margin,fn)=>{ const out=[]; let tries=0; n=Math.round(n*D); while(out.length<n&&tries<n*10){ tries++; const i=Math.floor(rnd()*P.N); const sd=rnd()<0.5?-1:1; const e=sd<0?P.wl[i]:P.wr[i]; const d=e+rr(minD,maxD);
      const x=P.x[i]+P.rx[i]*d*sd, z=P.z[i]+P.rz[i]*d*sd; if(!clearOfRoad(x,z,margin)) continue; const y=heightAt(x,z); const r=fn?fn(x,y,z,i,sd):{x,y,z,ry:rnd()*TAU}; if(r) out.push(r);} return out; };
  ...
  const PB=placeTrackProps(W,def,P,{heightAt,footprintClear}); const freeOfProps=(x,z,r)=>!PB.some(b=>Math.hypot(b.x-x,b.z-z)<b.r+r);
  ...
  if(def.theme==='night'){
    ...
    const wh=scatter(70,10,70,6,(x,y,z,i,sd)=>{ const ang=Math.atan2(P.tx[i],P.tz[i]); const w=rr(20,38),d=rr(16,28),h=rr(9,18); if(!footprintClear(x,z,ang,w,d,5)) return null; return {x,y:y-0.2,z,ry:ang,s:[w,h,d],c:pick([0xffffff,0xcfd6ff,0xffd9e8,0xd0fff5])}; });
